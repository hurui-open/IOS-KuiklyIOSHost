...
#import "../Thread/KuiklyRenderThreadManager.h"
#import "KRMemoryMonitor.h"
#import "../Core/KuiklyContextParam.h"
#import "../Core/KRUIKit.h" // [macOS]
#import "../Extension/Vendor/KRDisplayLink.h" // [macOS]
...
static std::mutex gLaunchStateLock;
static std::unordered_set<std::string> gLaunchedPages;

- (nonnull instancetype)initWithPageName:(nonnull NSString *)pageName {
    if (self = [super init]) {
        NSString *resolvedPageName = [pageName isKindOfClass:[NSString class]] ? [pageName copy] : @"";
        _pageName = resolvedPageName;
        pthread_rwlock_init(&_dataLock, NULL);
        _stageStartTimes = [NSMutableDictionary new];
        _stageDurations = [NSMutableDictionary new];
        _memoryMonitor = [[KRMemoryMonitor alloc] initWithPageName:resolvedPageName];
        _pageEnterDate = [NSDate date];
        _pageState = KRPageState_appActive;

        std::lock_guard<std::mutex> lock(gLaunchStateLock);
        if (gLaunchCount++ == 0) {
            _isFirstLaunchOfProcess = YES;
        }
        if (resolvedPageName.length > 0) {
            std::string pageKey = std::string(resolvedPageName.UTF8String ?: "");
            auto insertResult = gLaunchedPages.insert(pageKey);
