...
#import "Modules/KRSnapshotModule.h"
#import "Vendor/KRHttpRequestTool.h"
#import "Category/KRConvertUtil.h"
#import "../Handler/KuiklyRenderFrameworkContextHandler.h"
#import "../Core/KuiklyContextParam.h"
#import "../Performance/KRPerformanceManager.h"
#import "../Performance/KRPerformanceModule.h"
#import "Components/Base/KRWeakObject.h"
#import "Category/NSObject+KR.h"
#import "../Core/KuiklyRenderCore.h"
#import "../Handler/KuiklyRenderFrameworkContextHandler.h"
#import "../Core/KuiklyCoreDefine.h"
#import "Modules/KRBackPressModule.h"
...
- (instancetype)initWithPageName:(NSString *)pageName
    ...
    if (self = [super init]) {
        ...
        self.emitPagerVisibilityEvents = YES;
...
- (void)viewDidAppear {
    ...
    if (self.emitPagerVisibilityEvents) {
...
- (void)onReceiveApplicationDidBecomeActiveNotification:(NSNotification *)notification {
    __unused NSNotification *unusedNotification = notification;
    [self notifyApplicationDidBecomeActive];
}

- (void)onReceiveApplicationWillResignActiveNotification:(NSNotification *)notification {
    __unused NSNotification *unusedNotification = notification;
    [self notifyApplicationWillResignActive];
}
...
- (void)dealloc {
    NSSet<KRWeakObject *> *lifeCycleListeners = [_lifeCycleListenerSet copy];
    void (^cleanupBlock)(void) = ^{
        for (KRWeakObject *ele in lifeCycleListeners) {
            if ([ele.weakObject respondsToSelector:@selector(delegatorDealloc)]) {
                [ele.weakObject kr_invokeWithSelector:@selector(delegatorDealloc) args:nil];
            }
        }
    };
    if ([NSThread isMainThread]) {
